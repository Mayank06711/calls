...
import { fetchClientCloset, createClientOutfit } from '../../../../redux/thunks/booking.thunks';
import ClientClosetView from './ClientClosetView';
import SessionCatalogPanel from './SessionCatalogPanel';
...
export const PersonInfoCard = ({ detail, colors }) => {
  const { role } = detail;
...
export const BookingInfoCard = ({ booking, isUser, colors }) => {
...
export const PermissionTogglesCard = ({ permissions, onToggle, togglingPerm, colors }) => {
  const toggles = [
    {
      key: 'closet',
      label: 'My Closet',
...
export const ExpertTabs = ({ activeTab, setActiveTab, permissions, sessionActive, colors }) => {
  const tabs = [
...
export const ExpertTabContent = ({ activeTab, detail, bookingId, clientCloset, clientOutfits, permissions, sessionActive, colors, onTryOn, isClosetLoading, isOutfitsLoading }) => {
  if (!sessionActive || activeTab === 'profile') {
    return <StyleProfileTab detail={detail} colors={colors} />;
  }

  if (activeTab === 'closet') {
    // Show spinner while the API is checking permissions (avoids stale LockedTab)
    if (!permissions?.closet && isClosetLoading) {
      return <TabLoading label="Checking closet access..." colors={colors} />;
    }
  ...
  if (activeTab === 'outfits') {
    if (!permissions?.outfits && isOutfitsLoading) {
      return <TabLoading label="Checking outfits access..." colors={colors} />;
    }
  ...
  if (activeTab === 'catalog') {
    return <SessionCatalogPanel bookingId={bookingId} sessionActive={sessionActive} colors={colors} onTryOn={onTryOn} />;
  }

  return null;
};

const TabLoading = ({ label, colors }) => (
...
export const LockedTab = ({ label, colors }) => (
  <div className="bg-white dark:bg-dark-primary rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
    <div
...
export const StyleProfileTab = ({ detail, colors }) => {
...
export const OutfitsTab = ({ outfits, bookingId, clientCloset, permissions, colors }) => {
